<template>
  <div>
    ...
    <div class="row">
      <Card title="销售额" :strong="sale" unit="元" :huanbi="Math.abs(sale_huanbi)+'%'" :type="sale_huanbi>0?'up':'down'" />
      <Card title="收入" :strong="income" unit="元" :huanbi="Math.abs(income_huanbi)+'%'"
        :type="income_huanbi>0?'up':'down'" />
      <Card title="利润" :strong="profit" unit="元" :huanbi="Math.abs(profit_huanbi)+'%'"
        :type="profit_huanbi>0?'up':'down'" />
      <Card title="支付笔数" :strong="orders" unit="笔" :huanbi="Math.abs(orders_huanbi)+'%'"
...
export default {
  ...
  computed: {
    visitedViews () {
      return this.$store.state.dashboard.screenAll
    },
  },
  watch: {
    visitedViews: function (newValue) {
